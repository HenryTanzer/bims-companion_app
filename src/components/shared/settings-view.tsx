...
export type SettingsRole = 'student' | 'teacher' | 'admin'
...
export function SettingsView({
  ...
  function simpleToggle(current: boolean, setter: (v: boolean) => void, key: string) {
    const next = !current
    setter(next)
    setLS(key, next)
    vibrate()
  ...
  function handleTour() {
    const key = role === 'teacher' ? 'bims_teacher_tutorial_v1' : 'bims_student_tutorial_v1'
  ...
  const analyticsHref =
    role === 'student' ? `${basePath}/progress` : `${basePath}/analytics`
            ...
            {isStudent && (
              <>
                ...
                <ChevronRow
                  ...
                  onClick={() => router.push(`${basePath}/study-timer`)}
                />
              </>
            )}
          </CardContent>
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Re-launch the guided dashboard walkthrough
                </p>
              </div>
          ...
          <CardContent className="pt-1 pb-1 px-0">
            <div className="py-3 px-3 space-y-3">
              ...
              <div className="flex gap-2 pl-11">
                <Button size="sm" onClick={handleForceRefresh} className="gap-1.5">
                  <RefreshCw className="w-3.5 h-3.5" />
                  Force Refresh
                </Button>
                <Button size="sm" variant="outline" onClick={handleForceRefresh} className="gap-1.5">
                  <RefreshCw className="w-3.5 h-3.5" />
                  Check for Updates
                </Button>
              </div>
